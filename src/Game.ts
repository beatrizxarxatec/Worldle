...
export class Game {
    ...
    checkMisplacedLetters = ():void=> { // Se quitó porque no daba correcto el resultado
        ...
        for (let i=0; i<MAX_WORD_SIZE; i++){
            ...
            const pickedLetter = this.#pickedWord[i];
            ...
            if(pickedLetter != actualLetter){
                ...
                const numberOfCoincidencesActualWord = (this.#actualWord.match(pattern)||[]).length;
                const differenceOfCoincidences = Math.abs(numberOfCoincidencesActualWord - numberOfCoincidencesPickedWord);
                    let soFar = 0;
                    for (let j=0; j<=i; j++){
                        if(pickedLetter == this.#pickedWord[j]){
                            soFar++; 
                        }
                    }
